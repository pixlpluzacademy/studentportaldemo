...
import { courses, placements, students } from '@/lib/demo/seed'

type PlacementStatus = 'eligible' | 'applied' | 'interviewing' | 'selected' | 'rejected' | 'on-hold'
type ResumeStatus = 'pending' | 'approved' | 'revision-needed'
type InterviewStatus = 'not-scheduled' | 'scheduled' | 'completed' | 'selected' | 'rejected'
...
type PlacementRecord = {
  ...
  course?: string
  courseName?: string
  resume?: ResumeStatus | string
  resumeStatus?: ResumeStatus | string
  company?: string
  companyName?: string
  interview?: InterviewStatus | string
...
const placementRows = placements as PlacementRecord[]

const jobRows: JobRecord[] = [
  {
    id: 'job-1',
    title: 'Junior Digital Marketing Executive',
    company: 'Pixel Pluz Partner Company',
    course: 'Digital Marketing',
    location: 'Kochi',
    type: 'Full Time',
    salary: '₹18,000 - ₹25,000',
    link: 'https://example.com/job/digital-marketing-executive',
    status: 'open',
    applicants: 8,
    eligibleStudents: 12,
  },
  {
    ...
    type: 'Internship',
    salary: '₹8,000 - ₹12,000',
    link: 'https://example.com/job/frontend-developer-intern',
    status: 'open',
    applicants: 5,
    eligibleStudents: 9,
  },
...
function createEmptyJob(): JobRecord {
  return {
    id: `job-${Date.now()}`,
    title: '',
    company: '',
...
function getStudentName(item: PlacementRecord) {
  return item.studentName || item.student || 'Unnamed Student'
}

function getCourseName(item: PlacementRecord) {
  return item.courseName || item.course || 'Course not assigned'
}

function getCompanyName(item: PlacementRecord) {
  return item.companyName || item.company || 'Company not assigned'
}

function getResumeStatus(item: PlacementRecord) {
  return String(item.resumeStatus || item.resume || 'pending').toLowerCase()
}

function getInterviewStatus(item: PlacementRecord) {
  return String(item.interviewStatus || item.interview || 'not-scheduled').toLowerCase()
}

function getPlacementStatus(item: PlacementRecord) {
  return String(item.status || 'eligible').toLowerCase()
}
...
export default function Page() {
  ...
  const [jobs, setJobs] = useState<JobRecord[]>(jobRows)
  const [formJob, setFormJob] = useState<JobRecord>(() => createEmptyJob())
  const [editingJobId, setEditingJobId] = useState<string | null>(null)
  const [selectedJobId, setSelectedJobId] = useState(jobRows[0]?.id || '')
  ...
  const canManagePlacement =
    ...
    currentRoleId === 'admin' ||
    currentRoleId === 'placement' ||
    currentRoleId === 'placement_cell'
  ...
  const filteredPlacements = useMemo(() => {
    return placementRows.filter((item) => {
      ...
      const status = getPlacementStatus(item)

      const matchesSearch =
        !term ||
        getStudentName(item).toLowerCase().includes(term) ||
        getCourseName(item).toLowerCase().includes(term) ||
        getCompanyName(item).toLowerCase().includes(term) ||
        String(item.jobTitle || '').toLowerCase().includes(term) ||
  ...
  const eligibleCount = placementRows.filter((item) => getPlacementStatus(item) === 'eligible').length
  const interviewingCount = placementRows.filter((item) => getPlacementStatus(item) === 'interviewing').length
  const resumeApprovedCount = placementRows.filter((item) => getResumeStatus(item) === 'approved').length
  const openJobsCount = jobs.filter((item) => item.status === 'open').length

  const resetJobForm = () => {
    setFormJob(createEmptyJob())
  ...
  const saveJob = () => {
    ...
    const cleanJob: JobRecord = {
      ...
      salary: formJob.salary.trim(),
      link: formJob.link.trim(),
    }

    const exists = jobs.some((item) => item.id === cleanJob.id)

    if (exists) {
      setJobs(jobs.map((item) => (item.id === cleanJob.id ? cleanJob : item)))
      setNotice('Job details updated successfully.')
    } else {
      setJobs([cleanJob, ...jobs])
      setNotice('New job details added successfully.')
    }

    setSelectedJobId(cleanJob.id)
    resetJobForm()
  }
  ...
  const deleteJob = (jobId: string) => {
    ...
    setJobs(jobs.filter((item) => item.id !== jobId))

    if (selectedJobId === jobId) {
      setSelectedJobId(jobs.find((item) => item.id !== jobId)?.id || '')
    }
  ...
  const applyToJob = (item: JobRecord) => {
    if (item.status !== 'open') {
      setNotice('This job is not open for applications.')
      return
    }

    setNotice(`Student application flow selected for ${item.title}.`)
  }

  const updateStudentStatus = (item: PlacementRecord) => {
    if (!canManagePlacement) {
      setNotice('Only Placement Cell, Admin or Super Admin can update student placement status.')
      return
    }

    setNotice(`Placement status update selected for ${getStudentName(item)}.`)
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        ...
        <div className="border border-[#153e90]/25 bg-card px-4 py-3 text-sm">
          ...
          {canManagePlacement ? 'Placement management' : 'Student job view'}
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div className="border border-border bg-card p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="text-3xl font-bold">{eligibleCount}</div>
        ...
        <div className="border border-border bg-card p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="text-3xl font-bold">{resumeApprovedCount}</div>
        ...
        <div className="border border-border bg-card p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="text-3xl font-bold">{openJobsCount}</div>
          ...
          <div className="border border-border bg-card p-5">
            ...
            {filteredJobs.length === 0 && (
              <div className="mt-5 border border-border bg-background p-6 text-center text-sm text-muted-foreground">
                No job openings found for the selected filters.
              </div>
            )}
          </div>
              ...
              <table className="w-full min-w-[1180px] border-collapse text-sm">
                ...
                <tbody>
                  {filteredPlacements.map((item, index) => {
                    const status = getPlacementStatus(item)
                    const resume = getResumeStatus(item)
                    const interview = getInterviewStatus(item)
                    ...
                    return (
                      <tr key={item.id || `${getStudentName(item)}-${index}`} className="border-b border-border">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {item.jobTitle || 'Placement candidate'}
                              </p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3 text-muted-foreground">{getCompanyName(item)}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-2">
                            <button
                              type="button"
                              onClick={() => updateStudentStatus(item)}
                              className="border border-border p-2 hover:bg-accent"
                              title="Update Status"
                            >
                            ...
                            <button
                              type="button"
                              onClick={() => setNotice(`Placement report selected for ${getStudentName(item)}.`)}
                              className="border border-border p-2 hover:bg-accent"
                              title="Report"
                            >
